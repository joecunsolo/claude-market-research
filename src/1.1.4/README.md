...
Regenerates the HTML report directly from existing CSV files — no AI calls, no waiting. Use this whenever you want to refresh the report after manually editing the summary CSV, or after tweaking the report template.

Requires both `network_provider_summary.csv` and `network_provider_research_results.csv` to already exist.
