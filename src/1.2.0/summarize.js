...
import { chatExtract, checkConnection } from './llm-client.js';
...
// Function to extract clean numerical values from text using Ollama
...
Return only the classification word, nothing else.`;
      ...
      case 'Regional Market':
        prompt = `Extract the PRIMARY COUNTRY where this company is headquartered or has its main operations: "${fieldValue}"

Company: ${companyName}

Return ONLY the country name (e.g., "United States", "Germany", "United Kingdom", "France", "Japan").
Use the full country name, not abbreviations.
If the company operates globally but is clearly based in one country, return that country.
If multiple countries are mentioned equally, return the headquarters country.
If unclear or not mentioned, return "N/A".
Keep the response to just the country name, nothing else.`;
        break;
...
Return only the tier name, nothing else.`;
    ...
    const result = await chatExtract(prompt);
    return result;
  } catch (error) {
...
// Main function
async function main() {
  console.log(chalk.bold.magenta('\n📊 Network Provider Summary Generator (Ollama)\n'));
  ...
  for (let i = 0; i < companies.length; i++) {
    ...
    const summary = {
      ...
      'Primary Country': await extractValue(companyName, 'Regional Market', company['Regional Market'] || '')
    };
  ...
  const csvWriter = createObjectCsvWriter({
    ...
    header: [
      ...
      { id: 'Primary Country', title: 'Primary Country' }
    ]
  });
  ...
  try {
    generateHTMLReport(summaries, companies, HTML_REPORT_FILE);
    htmlSpinner.succeed(chalk.green(`✓ Interactive report saved to ${HTML_REPORT_FILE}`));
  } catch (error) {
    htmlSpinner.fail(chalk.red(`Failed to generate HTML report: ${error.message}`));
  }

  console.log(chalk.bold.green('\n✅ Summary generation complete!\n'));
  console.log(chalk.cyan(`📄 Summary CSV: ${OUTPUT_FILE}`));
  console.log(chalk.cyan(`📊 Interactive Report: ${HTML_REPORT_FILE}`));
  console.log(chalk.yellow('\nℹ️  The summary contains clean numerical values extracted from the research data.'));
  console.log(chalk.yellow('ℹ️  Open the HTML report in your browser for interactive charts and data exploration.'));
  console.log(chalk.gray('━'.repeat(60) + '\n'));
}
