...
import ora from 'ora';
import { chatWithSearch, checkConnection } from './llm-client.js';
...
async function searchCompanyData(companyName, query) {
  try {
    ...
    const prompt = `Provide information about ${companyName} ${query}. Give a concise, factual answer with specific numbers or data where available. Keep response under 100 words.`;

    const textContent = await chatWithSearch(prompt, searchQuery);
...
async function getNewsArticles(companyName) {
  try {
    const searchQuery = `${companyName} latest news telecommunications`;
    const prompt = `Based on the search results, list the 3-5 most recent and relevant news articles about ${companyName}. For each article, provide:
1. Article title
...
Keep it concise and factual.`;
    ...
    const textContent = await chatWithSearch(prompt, searchQuery, 800);
...
async function main() {
  ...
  console.log(chalk.bold.magenta(`\n📊 Network Provider Research Tool (Ollama)${isUpdateMode ? ' - Update Mode' : ''}\n`));
